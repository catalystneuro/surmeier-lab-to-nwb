#!/usr/bin/env python3
"""
Figure 3 Somatic Excitability Reproduction Script

This script reproduces Figure 3 from Zhai et al. 2025 using NWB files containing
somatic excitability data from indirect pathway SPNs (iSPNs). It loads current clamp
recordings, calculates rheobase values, and generates publication-quality F-I
(frequency-intensity) curves.

The analysis adapts the original MATLAB/Python code to work with NWB structured data,
providing rheobase measurements and action potential counting for iSPN excitability.

Usage:
    python figure_scripts/figure_3_somatic_excitability_reproduction.py

Output:
    - analysis_outputs/figure_3/figure_3_reproduction_report.md
    - analysis_outputs/figure_3/figure_3_fi_curves.png
    - analysis_outputs/figure_3/figure_3_rheobase_comparison.png
"""
...
def load_somatic_excitability_data(nwb_dir: Path) -> pd.DataFrame:
    ...
    ----------
    nwb_dir : Path
        Directory containing Figure 3 somatic excitability NWB files
    ...
    """
    nwb_files = list(nwb_dir.glob("F3++SomExc++iSPN++*.nwb"))
...
def create_fi_curves(df: pd.DataFrame) -> Tuple[plt.Figure, plt.Axes]:
    """
    Create frequency-intensity (F-I) curves matching paper style for Figure 3 (iSPN).
    ...
    for condition in ["LID off-state", "LID on-state", "LID on-state with sul (iSPN)"]:
    ...
    ax.set_title("Figure 3: iSPN Somatic Excitability (F-I Curves)", fontsize=14, pad=20)
...
def create_rheobase_comparison(cell_stats: pd.DataFrame) -> Tuple[plt.Figure, plt.Axes]:
    """
    Create rheobase comparison to match paper style for Figure 3 (iSPN).
    ...
    condition_order = ["LID off-state", "LID on-state", "LID on-state with sul (iSPN)"]
    condition_labels = ["off", "on", "on + sul"]
    ...
    if not plot_data:
        ...
        ax.set_title("Figure 3: iSPN Rheobase Comparison", fontsize=14, pad=20)
        return fig, ax
...
def generate_markdown_report(cell_stats: pd.DataFrame, output_dir: Path) -> str:
    ...
    markdown_content = f"""# Figure 3 Somatic Excitability Analysis Report

**Generated:** {timestamp}
**Data Source:** NWB files with intracellular current clamp recordings
**Analysis:** F-I curves and rheobase measurements for iSPN somatic excitability

## Executive Summary

This report presents the reproduction of Figure 3 from Zhai et al. 2025, demonstrating
somatic excitability changes in indirect pathway spiny projection neurons (iSPNs) across
different LID (L-DOPA-induced dyskinesia) states.

### Key Findings

- **Total Cells Analyzed:** {len(valid_data)}
- **Conditions:** {len(valid_data['condition'].unique()) if not valid_data.empty else 0}
- **Total Recordings:** {valid_data['n_recordings'].sum() if not valid_data.empty else 0}

## Methodology

### Experimental Protocol
Current clamp recordings from iSPN somata with step current injections:
...
"""
        ...
        markdown_content += "No valid data available for analysis.\n\n"

    markdown_content += f"""
## Figures

### F-I Curves
![F-I Curves](figure_3_fi_curves.png)

**F-I Curves:** Frequency-intensity relationships showing action potential firing rates
in response to depolarizing current steps across LID conditions in iSPNs.

### Rheobase Comparison
![Rheobase Comparison](figure_3_rheobase_comparison.png)
...
- `figure_3_fi_curves.png` - F-I curve analysis
- `figure_3_rheobase_comparison.png` - Rheobase statistical comparison
- `figure_3_reproduction_report.md` - This detailed report

---

*Report generated by Figure 3 Somatic Excitability Reproduction Script*
"""

    # Write report
    report_path = output_dir / "figure_3_reproduction_report.md"
...
def main():
    """Main function to orchestrate the Figure 3 reproduction analysis."""

    # Set up paths
    base_dir = Path(__file__).parent.parent
    nwb_dir = base_dir / "nwb_files" / "somatic_excitability" / "figure_3"
    output_dir = base_dir / "analysis_outputs" / "figure_3"

    # Ensure output directory exists
    output_dir.mkdir(parents=True, exist_ok=True)

    print("=" * 80)
    print("FIGURE 3 SOMATIC EXCITABILITY REPRODUCTION ANALYSIS")
    print("=" * 80)
    ...
    fig1_path = output_dir / "figure_3_fi_curves.png"
    ...
    fig2_path = output_dir / "figure_3_rheobase_comparison.png"
    ...
    print(f"Total Recordings: {valid_stats['n_recordings'].sum() if not valid_stats.empty else 0}")
    print(f"Conditions: {len(valid_stats['condition'].unique()) if not valid_stats.empty else 0}")
    ...
    print("\n" + "=" * 80)
    print("Figure 3 reproduction demonstrates NWB-powered iSPN somatic excitability analysis!")
