...
def convert_session_to_nwbfile(session_folder_path: Path, condition: str, verbose: bool = False) -> NWBFile:
    ...
    session_specific_metadata = {
        "NWBFile": {
            ...
            "identifier": f"zhai2025_fig6_dendritic_{session_folder_path.name}_{condition.replace(' ', '_')}",
    ...
    intracellular_recording_table.add_column(
        name="m1r_treatment", description="M1R antagonist treatment status for this recording"
    )
    ...
    for recording_id, recording_folder in recording_id_to_folder.items():
        ...
        # Find electrophysiology XML file (exact name from Figure 6 notes)
        electrophysiology_xml_file = recording_folder / f"{recording_folder.name}_Cycle00001_VoltageRecording_001.xml"
        ...
        # Update electrode description for iSPN dendritic recording (Figure 6)
        ...
        intracellular_metadata["Icephys"]["CurrentClampSeries"][icephys_metadata_key].update(
            {
                ...
                "description": (
                    ...
                    f"M1R treatment: {'THP (3 mg/kg, i.p.) + VU 0255035 (5 μM)' if 'antagonist' in condition else 'Saline control'}"
                ),
            }
        )
        ...
        # Determine M1R treatment status
    ...
    repetitions_table.add_column(
        name="m1r_treatment", description="M1R antagonist treatment status for this repetition"
    )
    ...
    # Step 4: Build experimental conditions table (group all repetitions by M1R treatment condition)
    ...
    experimental_conditions_table.add_column(
        name="condition", description="Experimental condition for M1R antagonist study in LID"
    )
    experimental_conditions_table.add_column(name="m1r_treatment", description="M1R antagonist treatment details")

    m1r_treatment = "THP + VU 0255035" if "antagonist" in condition else "Control"
    experimental_condition_index = nwbfile.add_icephys_experimental_condition(
        repetitions=repetition_indices, condition=condition, m1r_treatment=m1r_treatment
    )
...
if __name__ == "__main__":
    ...
    nwb_files_dir = root_dir / "nwb_files" / "figure_6_dendritic_excitability"
