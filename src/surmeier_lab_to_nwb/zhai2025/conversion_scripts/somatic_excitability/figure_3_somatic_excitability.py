...
def convert_session_to_nwbfile(session_folder_path: Path, condition: str) -> NWBFile:
    ...
    if condition == "LID off-state":
        ...
        state = "ON"
        pharmacology = "none"
    elif condition == "LID on-state with sul (iSPN)":
        state = "ON"
        pharmacology = "D2RA"
    else:
    ...
    session_id = generate_canonical_session_id(
        ...
        genotype="WT",  # Wild-type for all Figure 3
        timestamp=timestamp,
    )
    ...
    script_template = session_metadata_template["figure_3_somatic_excitability"]
    ...
    if "sul" in condition_underscore and "pharmacology_conditions" in script_template["NWBFile"]:
        if "sulpiride" in script_template["NWBFile"]["pharmacology_conditions"]:
            pharmacology_addition = " " + script_template["NWBFile"]["pharmacology_conditions"]["sulpiride"]
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert Figure 3 somatic excitability data to NWB format")
