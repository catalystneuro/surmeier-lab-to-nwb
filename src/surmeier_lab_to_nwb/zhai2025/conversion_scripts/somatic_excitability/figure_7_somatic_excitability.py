"""
Figure 7 Somatic Excitability Conversion Script - Zhai et al. 2025
==============================================================

This script converts somatic excitability data from Figure 7 of Zhai et al. 2025
        ...
        interface_metadata["Icephys"]["IntracellularElectrodes"][icephys_metadata_key].update(
            {
                "name": electrode_name,
                "description": (
                    f"Whole-cell patch clamp electrode recording from CDGI knockout {cell_type} soma in the dorsolateral striatum - "
        ...
        interface_metadata["Icephys"]["CurrentClampSeries"][icephys_metadata_key].update(
            {
                "name": series_name,
                "description": (
                    f"Current clamp recording from CDGI knockout {cell_type} - {condition_human_readable} - "
...
if __name__ == "__main__":
    ...
    parser = argparse.ArgumentParser(description="Convert Figure 7 somatic excitability data to NWB format")
    ...
    nwb_files_dir = root_dir / "nwb_files" / "somatic_excitability" / "figure_7"
    ...
    for condition in conditions:
        ...
        session_iterator = tqdm(
            session_folders,
            desc=f"Converting Figure7 SomaticExcitability {condition}",
            unit=" session",
        )
