...
def convert_session_to_nwbfile(
    session_date: str, session_number: int, animal_id: int, genotype: str, verbose: bool = False
) -> NWBFile:
    """
    ...
    cached_data_path = Path(__file__).parent / "assets" / "figure_7_aim_processed_data.csv"
    aim_interface = AIMBehavioralDynamicTableInterface(
        processed_data_csv_path=cached_data_path,
        session_date=session_date,
...
if __name__ == "__main__":
    ...
    aim_excel_data_file_path = base_dir / "assets" / "AIM testing_CDGI KO.xlsx"
    genotype_path = base_dir / "assets" / "data_connections_D2_figures_3_4_6_7_8.csv"
    cached_data_path = base_dir / "assets" / "figure_7_aim_processed_data.csv"
    ...
    # Save processed data for interface use
    pivot_df.to_csv(cached_data_path, index=False)
    if verbose:
